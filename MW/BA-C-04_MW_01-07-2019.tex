...
\begin{document}
	...
	Weiterhin werden in einem Koaxialkabel stehende Wellen herbeigefürt und  mithilfe der Frequenzabstände, unter denen Resonanzen auftreten, die relative Permittivität der zwei verwendeten Kabel bestimmt.
	Mithilfe dieser wird vermutet, dass es sich bei den Materialien der Dielectrica der beiden Kabel und Polyethylenschaum und Teflon handelt.
  ...
  \section{Theorie}
	...
	Wird angenommen, dass ein Signal über Leitung 1 einläuft, so ist schnell festzustellen, dass das Signal über die beiden direkten Wege dieselbe Strecke zurücklegt, also keine Laufzeitdifferenz aufweist, sodass an 4 konstruktive Interferenz herrscht, das Signal also übertragen wird. %TODO erwähnen, dass das auch im Kreis laufen kann und dann destruktiv ist?
  ...
  Bei den Messungen wird jeweils bei fester Eingangsleistung die Ausgangsspannung der Diode gemessen.
	...
	Wann immer stehende Wellen auftreteten, kommt ausschließlich die am Koppler reflektierte Leistung an der Messdiode an, sodass die gemessene Spannung hier geringer ist, als wenn Wellen die Messleitung wieder über den Koppler verlassen können.
	%TODO habe ich das richtig verstanden?
	Der Zirkulator sorgt dafür, dass näherungsweise keine Wellen vom Generator direkt an der Messdiode ankommen und keine am Koppler reflektierten Wellen wieder im Mikrowellengenerator enlaufen.
...
\end{table}
	\subsubsection*{Diskussion}
	%TODO sieht qualitativ wie Airy fkt aus.
	...
	Außerdem konnten stehende Wellen in zwei verschiedenen Koaxialkabeln nachgewiesen werden und die relative Permittivitäten der Dielectrica der beiden Kabel bestimmt werden.
	Ein Vergleich mit den Permittivitäten von Stoffen, die in Koaxialkabeln üblicherweise als Dielectricum verwendet werden, ließ grobe Schlüsse auf das verwendete Material zu.
