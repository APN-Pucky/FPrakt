  ...
  %Die Differenz der Brechungsindizes ist dabei direkt abhängig vom Ordnungsgrad, weshalb sie als ein Maß für diesen verwendet werden kann.
    ...
    \end{figure}
		...
		Die Bestimmung des Kontrast erfolgt analog zu \cref{eq_kontrast} in \cref{ss_malus}, jedoch mit  den gemessenen intensitäten $I_\text{max}=\input{res/intens_max}$ und $I_\text{min}=\input{res/intens_min}$, respektiv für einen weißen und schwarzen \textit{Blank Screen}, sodass sich ein Kontrast von $C=\input{res/intens_kontrast}$ ergibt.
			...
			Die Tatsache, dass $I_\text{min} = \input{res/intens_min}$ sich deutlich von Null unterscheidet zeigt, dass der LC-Modulator deutlich schlechter linear polarisiertes Lich ausgibt, als der Laser.
...
\end{table}
			...
			Wenn man die gemessenen Pixelabstände in \cref{tb_pixel} und \cref{tb_gitter} mit der Herstellerangabe gemäß \cite{Handbuch} von \SI{32}{\micro \meter}, die auf \SI{\pm 3}{mm} genau bestimmt werden können soll, vergleicht, stellt man fest, dass dies innerhalb der Messunsicherheiten der Fall ist. %TODO geiler Satz.
...
\begin{figure}[H]
	...
	\end{figure}
			...
			Die Polarisation dreht sich mit dem Grauwert und die Exzentrizität wird bei einem mittleren Grauzwert maximal.
			...
			%TODO irgendwas zu der Exzentrizität?
...
\begin{figure}[H] %TODO einsam auf der Seite, random leerseite
	...
	\end{figure}
			...
			Der oben abgeschätzte Füllfaktor von \SI{9,9\pm 0,7}{\percent} steht im Widerspruch zu \cite{Handbuch}, welches eine Angabe von etwa \SI{55}{\percent} macht.
			Dies liegt an den ebenfalls unterschiedlichen Tastgraden, die laut Handbuch nämlich in horizontaler Richtung $5/6$ und in vertikaler $2/3$ betragen sollen.
			Die gemessenen Tastgrade sind deutlich geringer.
			Dies könnte unter anderem auf die Versuchslimitierung durch die Ordnungen des Pixelgitters zurückgeführt werden:
