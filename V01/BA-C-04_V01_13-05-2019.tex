...
\begin{document}
  ...
  \section{Theorie}
	...
	Da $\gamma$-Strahlung im Allgemeinen geringe Wechselwirkung mit Materie zeigt, muss für eine zielführende Messrate ein Stoff mit hoher Dichte (ein Festkörper) oder ein großes Volumen eines einfach zugänglichen Stoffes (Wasser) verwendet werden. % er hatte hier auch den Xenondetektor erwähnt, weil das seine Arbeitsgruppe ist, aber die detektieren ja keine gamma-strahlung. Außerdem ist das flüssiges xenon also quasi Wasser.
	...
	\subsection{Energieauflösung und mittlere Ionisationsenergie} \label{ss_ion}
...
% Photomultiplier? Was braucht man da sonst noch?->Lichtleiter, Vorverstärker.
	...
	Damit das Signal über Koaxialkabel mit BNC-Steckverbindung übertragen werden kann, wird es bei beiden Detektoren vorverstärkt. %TODO er hat da *BNC drangeschrieben. Laut Wikipedia darf man aber scheinbar nicht BNC-Kabel sagen, weil BNC nur der Stecker ist und Kaxial ist sowieso true.
...
Hierfür ist die Formel, welche in \cref{ss_ion} hergeleitet wurde:
...
Für den NaI-Detektor ist dies zu erkennen und sie würde auf etwa \SI{0,7}{keV} geschätzt werden. %TODO hier finde ich keine Bandlücke zum vergleichen
...
Dies liegt nicht unweit der Bandlücke von Germanium bei \SI{0,66}{eV} \cite{bandlueckeGE}. %TODO muss ich hier noch irgendwie mehr zu sagen?
...
\begin{table}[H]
		...
		\begin{tabular}{c | c | c | c | c }
			 ...
			 Isotop& $^{60}$Co & $^{137}$Cs& $^{241}$Am & $^{109}$Cd \\ \hline
$E_\gamma$ in \si{keV} & \SI{1332}{}&\SI{662}{} &\SI{60}{}&\SI{88}{} \\
			 & \SI{1173}{} && \\
			 Aktivität $A(t')$ in \si{Bq} & \SI{1.18e3}{}&\SI{2.38e3}{}&\SI{3.35e3}{}&\SI{1.17e4}{}\\
			 Referenz Aktivität $A(t_r)$ in \si{Bq} & \SI{2.16e3}{}&\SI{2.64e3}&\SI{3.37e3}{} & \SI{1.40e4}{}\\
		\end{tabular}
		\caption{
		 Referenz (14.10.2014) und aktuelle Aktivitäten (13.05.2019).
		}
		\label{tb_eff_lit}
\end{table}
...
\subsubsection{Diskussion}
	...
	Besonders gut zu erkennen sind $^{214}$Pb und $^{214}$Bi (vgl. \nameref{s_anhang}). %TODO "Wie erkenne ich das?" Sehe ich ein, dass man das dem Bild nicht so direkt entnimmt. Ideen?
	Anhand der Zerfallsreihen lässt sich damit die Probe der Uran-Radium-Reihe zuordnen, weshalb es sich bei dem Erz wohl ursprünglich um $^{238}$U handelte.
	...
	Insgesamt lässt sich sagen, dass alle Messungen erfolgreich durchgeführt werden konnten und die in den Spektren gemessenen Peaks den Literaturwerten zugeordnet werden konnten.
